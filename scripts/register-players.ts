...
// 생년월일 파싱 (YYMMDD → Date)
function parseBirth(s: string): Date | null {
  if (!s || s === '-') return null;
...
function normalizePos(s: string): string | null {
  ...
  if (upper.startsWith('G') || upper === '가드') return 'G';
  if (upper.startsWith('F') || upper === '포워드') return 'F';
  if (upper.startsWith('C') || upper === '센터') return 'C';
  return upper;
}
...
async function main() {
  ...
  for (const teamData of allTeams) {
    // 팀 ID 조회
    const team = await prisma.$queryRaw<{id: bigint}[]>`
      SELECT id FROM teams WHERE name = ${teamData.teamName} LIMIT 1
    `;
    if (team.length === 0) {
      console.log(`❌ 팀 없음: ${teamData.teamName}`);
      continue;
    }
    const teamId = team[0].id;

    // tournament_team ID 조회
    ...
    `;
    if (tt.length === 0) {
      console.log(`❌ 대회 미등록: ${teamData.teamName}`);
      continue;
    }
    const tournamentTeamId = tt[0].id;

    console.log(`\n📋 ${teamData.teamName} (teamId=${teamId}, ttId=${tournamentTeamId})`);

    for (const p of teamData.players) {
      const birth = parseBirth(p.birth);
      const pos = normalizePos(p.position);
      const email = `${teamData.teamName.replace(/\s/g, '')}_${p.jerseyNumber ?? p.name}@bdr.local`;

      // 1. users 테이블에 유저 생성 (없으면)
      let user = await prisma.$queryRaw<{id: bigint}[]>`
      ...
      if (user.length > 0) {
        ...
        // name으로도 검색 (이전 실행에서 다른 이메일로 생성됐을 수 있음)
        const byName = await prisma.$queryRaw<{id: bigint}[]>`
          SELECT id FROM users WHERE name = ${p.name} AND nickname LIKE ${`${teamData.teamName}%`} LIMIT 1
        `;
        if (byName.length > 0) {
          userId = byName[0].id;
        } else {
          try {
            await prisma.$executeRaw`
              INSERT INTO users (email, name, nickname, position, birth_date, created_at, updated_at)
              VALUES (${email}, ${p.name}, ${`${teamData.teamName} #${p.jerseyNumber ?? '?'}`}, ${pos}, ${birth}, NOW(), NOW())
            `;
          } catch (e: any) {
            // ignore
          }
          const newUser = await prisma.$queryRaw<{id: bigint}[]>`
            SELECT id FROM users WHERE email = ${email} OR (name = ${p.name} AND nickname LIKE ${`${teamData.teamName}%`}) LIMIT 1
          `;
          if (newUser.length === 0) {
            console.log(`  ❌ ${p.name} 유저 생성 실패`);
            continue;
          }
          userId = newUser[0].id;
        }
      ...
      // 2. team_members 등록 (없으면)
      const tmExists = await prisma.$queryRaw<{id: bigint}[]>`
        SELECT id FROM team_members WHERE team_id = ${teamId} AND user_id = ${userId} LIMIT 1
      `;
      if (tmExists.length === 0) {
        try {
          await prisma.$executeRaw`
            INSERT INTO team_members (team_id, user_id, role, jersey_number, position, status, joined_at, created_at, updated_at)
            VALUES (${teamId}, ${userId}, 'player', ${p.jerseyNumber}, ${pos}, 'active', NOW(), NOW(), NOW())
          `;
        } catch (e: any) {
          // unique 제약 등 무시
        }
      ...
      // 3. tournament_team_players 등록 (없으면)
      const ttpExists = await prisma.$queryRaw<{id: bigint}[]>`
        SELECT id FROM tournament_team_players
        WHERE tournament_team_id = ${tournamentTeamId} AND user_id = ${userId}
        LIMIT 1
      `;
      if (ttpExists.length === 0) {
        try {
          await prisma.$executeRaw`
            INSERT INTO tournament_team_players (tournament_team_id, user_id, jersey_number, position, role, is_starter, created_at, updated_at)
            VALUES (${tournamentTeamId}, ${userId}, ${p.jerseyNumber}, ${pos}, 'player', false, NOW(), NOW())
          `;
          console.log(`  ✅ #${p.jerseyNumber ?? '?'} ${p.name}`);
        } catch (e: any) {
          console.log(`  ⚠️ #${p.jerseyNumber ?? '?'} ${p.name} (${e.meta?.message?.substring(0,50) ?? e.message})`);
        }
      } else {
        console.log(`  ℹ️  #${p.jerseyNumber ?? '?'} ${p.name} (이미 등록)`);
      }
    }
  }
