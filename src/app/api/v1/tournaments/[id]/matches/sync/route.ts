...
async function handler(req: NextRequest, ctx: AuthContext, tournamentId: string) {
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // 3. Play-by-Play upsert (local_id로 중복 방지)
      if (play_by_plays && play_by_plays.length > 0) {
        for (const pbp of play_by_plays) {
          const pbpData = {
    ...
    console.error(`[match-sync] Match ${match.server_id} failed:`, err);
