              ...
              <Card key={t.id.toString()}>
                <div className="mb-3 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {t.team.city && (
                      <p className="text-xs text-[#A0A0A0]">{t.team.city}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="mb-3 grid grid-cols-3 gap-2 text-center text-sm">
                  <div>
                    ...
                    <p className="text-xs text-[#666666]">승</p>
                  </div>
                  <div>
                    <p className="font-bold text-[#EF4444]">{t.losses ?? 0}</p>
                    <p className="text-xs text-[#666666]">패</p>
                  </div>
                  <div>
                    <p className="font-bold">{winPct}%</p>
                    <p className="text-xs text-[#666666]">승률</p>
                  </div>
                </div>

                {t.players.length > 0 && (
                  <div>
                    <p className="mb-1 text-xs text-[#666666]">선수 ({t.players.length}명)</p>
                    <div className="flex flex-wrap gap-1">
                      {t.players.map((p) => (
                        <span
                          key={p.id.toString()}
                          className="rounded-full bg-[#252525] px-2 py-0.5 text-xs"
                        >
                          {p.users?.nickname ?? "선수"}
