  ...
  return (
    <div>
      ...
      <div className="mb-4 flex gap-2 overflow-x-auto pb-1">
        <Link href="/community" className="shrink-0 whitespace-nowrap rounded-full bg-[rgba(0,102,255,0.12)] px-4 py-2 text-sm font-medium text-[#F4A261]">전체</Link>
        {Object.entries(categoryMap).map(([key, val]) => (
          <Link key={key} href={`/community?category=${key}`} className="shrink-0 whitespace-nowrap rounded-full border border-[#E8ECF0] px-4 py-2 text-sm text-[#6B7280] hover:text-[#111827]">{val.label}</Link>
        ))}
      </div>
